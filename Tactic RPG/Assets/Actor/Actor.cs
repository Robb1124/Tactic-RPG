...
[RequireComponent(typeof(BuffsSystem))]
public class Actor : MonoBehaviour
{
    ...
    public void LoadAttackConfig(int abilityIndex)
    {
        ...
        animationString = currentAbility.animationString;
        DisplayAttackRange();
    }
    ...
    private IEnumerator TargetPreview(BaseTile targetTile)
    {
        ...
        if (!attackIsCancelled)
        {
            ...
            animator.SetTrigger(animationString); //pour choisir l'animation, j'ai passer un string qui vient du ability sheet.
        }        
    }
    ...
    {
        BuffType[] buffTypesToApply = currentAbility.attackConfig.buffsToApply;
        BuffsApplication[] howToApplyBuffs = currentAbility.attackConfig.howToApplyBuffs;
        int[] buffsDurations = currentAbility.attackConfig.buffsTurnDuration;

        for (int i = 0; i < buffTypesToApply.Length; i++)
        {
            switch (howToApplyBuffs[i])
            {
                case BuffsApplication.TargetsOnly:
                    
                    for (int j = 0; j < currentTargets.Count; j++)
                    {
                        if(currentTargets[j] != null && (currentTargets[i] != this || selfCastAllowed))
                        {
                            currentTargets[j].GetComponent<BuffsSystem>().AddBuff(buffTypesToApply[i], buffsDurations[i]);
                        }
                    }
                                       
                    break;
                case BuffsApplication.CasterOnly:
                    this.GetComponent<BuffsSystem>().AddBuff(buffTypesToApply[i], buffsDurations[i]);
                    break;
                case BuffsApplication.AllAllies:
                    break;
                case BuffsApplication.AllEnemies:
                    break;
            }
        }
    }
    ...
    public void ReceiveOrRemoveBuffEffect(StatsChange statsChange, bool add)
    {
        if (add)
        {
            for (int i = 0; i < statsChange.affectedStats.Length; i++)
            {
                switch (statsChange.affectedStats[i])
                {
                    case StatsAffected.HP:
                        int savedMaxHp = maxHp;
                        maxHp = Mathf.RoundToInt(maxHp * statsChange.affectedByPercentage[i] + statsChange.affectedByValue[i]);
                        int maxHpAdded = maxHp - savedMaxHp;
                        currentHp += maxHpAdded;
                        break;
                    case StatsAffected.DMG:
                        baseDmg = Mathf.RoundToInt(baseDmg * statsChange.affectedByPercentage[i] + statsChange.affectedByValue[i]);
                        break;
                }
            }
        }
        ...
        {
            for (int i = 0; i < statsChange.affectedStats.Length; i++) //pour l'instant ca fonctionne, todo : faire une structure qui va permettre de combiner plusieurs buffs : exemple plusieurs health buff
                //pour pas qu'on ajoute 30% de bonus hp sans prendre en compte que des bonus hp sont deja octroyer. Donc peut-etre sauvegarde les stats de base et influer directement sur lui. (Design decision)
            {
                switch (statsChange.affectedStats[i])
                {
                    case StatsAffected.HP:
                        int savedMaxHp = maxHp;
                        maxHp = Mathf.RoundToInt((maxHp - statsChange.affectedByValue[i]) / statsChange.affectedByPercentage[i]);
                        currentHp = Mathf.Clamp(currentHp, 0, maxHp);
                        break;
                    case StatsAffected.DMG:
                        baseDmg = Mathf.RoundToInt((baseDmg - statsChange.affectedByValue[i]) / statsChange.affectedByPercentage[i]);
                        break;
                }
            }
        } 
